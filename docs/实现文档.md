...
修改型 cookie 请求额外校验 `Origin`，降低 CSRF 风险。生涯助手不继承门户管理员名单；jAccount 首次登录默认是学生，管理员授权独立保存在生涯数据库中。管理员同时保留填写问卷和查看个人报告的能力。
...
- 生涯助手不读取门户的管理员标记；jAccount 默认按学生身份进入，管理员由生涯项目单独授权，并同时具备学生端功能。
